...
4. **Smart Testing Pyramid & Process Guard:**
   ...
   - **Targeted E2E Smoke Test (Timeout 30s):** Executed upon feature completion. Must meet **Zero Network Errors (HTTP $\ge 400$)** and cleanly kill background processes.
   - **Evidence Ledger:** Record test metadata into `.brain/verification_ledger.json` instead of dumping raw stdout to chat context.
5. **Sacred Prompt Caching Protection:**
...
6. **Database Integrity & Explicit Foreign Key Hints:**
...
7. **Systematic Error Classification & Failed-First-Fix Rule:**
...
8. **Continuous Learning & Autonomous Skill Synthesis:**
   - Automatically record root causes and proven solutions in `.brain/learnings.md` following successful bug fixes.
   - Synthesize complex reusable solutions into `skills/custom/[skill-name]/SKILL.md` compliant with `agentskills.io`.
   - Query `.brain/learnings.md` via `.\scripts\brain-query.ps1` before authoring new plans.
9. **Strict Automated Guardrails & Pre-flight Gate:**
