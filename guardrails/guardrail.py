...
def execute_commands(repo: Path, commands: list[Command]) -> list[Finding]:
    ...
    for command in commands:
        resolved_path = shutil.which(command.argv[0])
        if resolved_path is None:
            ...
            continue
        argv = [resolved_path] + command.argv[1:]
        completed = subprocess.run(argv, cwd=repo, text=True, capture_output=True, encoding="utf-8")
